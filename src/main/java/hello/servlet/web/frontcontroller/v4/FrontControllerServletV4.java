...
public class FrontControllerServletV4 extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        ControllerV4 controller = controllerMap.get(requestURI); // 미리 URI로 매핑된 Controller 를 불러온다
        ...
        Map<String, Object> model = new HashMap<>(); // v3 -> v4 넘어오면서 추가됨
        String viewName = controller.process(paramMap, model); // 각 Controller에게 넘겨주면 로직을 수행하고, 논리이름 반환

        MyView view = viewResolver(viewName); // 뷰 반환 하는 메서드 사용(논리 이름에 경로와 확장명.jsp 추가해줌)

        view.render(model, request, response); // jsp 는 request 객체를 사용하기에 model의 parsing 이 필요함 (model 같이 넘겨줌)
    }
